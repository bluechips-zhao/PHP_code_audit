...
ERROR_LOG_GLOBAL="${MEMORY_DIR}/error.log"

# ── 依赖检查 ──
if ! command -v sqlite3 >/dev/null 2>&1; then
    echo "❌ sqlite3 not found. Install: apt-get install sqlite3 / brew install sqlite / yum install sqlite" >&2
    exit 1
fi

if ! command -v jq >/dev/null 2>&1; then
    echo "⚠️  jq not found. Some commands may fail. Install: apt-get install jq / brew install jq" >&2
fi

# ── 错误日志路径选择 ──
_get_error_log() {
    if [ -n "${WORK_DIR:-}" ] && [ -d "${WORK_DIR}/.audit_state" ]; then
        echo "${WORK_DIR}/.audit_state/error.log"
    else
        echo "$ERROR_LOG_GLOBAL"
    fi
}
...
_check_db_writable() {
    ...
    if [ -f "$db" ] && [ ! -w "$db" ]; then
        echo "❌ Database not writable: $db (check permissions)" >&2
        return 1
    fi
    ...
    if [ ! -w "$db_dir" ]; then
        echo "❌ Database directory not writable: $db_dir" >&2
        return 1
    fi
...
sql() {
    local db="$1"; shift
    local err_log
    err_log=$(_get_error_log)
    _check_db_writable "$db" || return 1
    sqlite3 -batch -json "$db" ".timeout 5000" "$@" 2>>"$err_log"
}
...
sql_exec() {
    ...
    err_log=$(_get_error_log)
    _check_db_writable "$db" || return 1
    sqlite3 -batch "$db" ".timeout 5000" "$@" 2>>"$err_log"
}
