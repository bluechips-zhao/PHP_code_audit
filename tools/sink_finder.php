...
// ============================================================
// Sink 定义 — 从 sink_registry.json 加载（单一数据源）
// ============================================================

function loadSinkRegistry(): array
{
    $registryPaths = [
        __DIR__ . '/../schemas/sink_registry.json',
        __DIR__ . '/schemas/sink_registry.json',
        dirname(__DIR__) . '/schemas/sink_registry.json',
    ];
    ...
    if ($registryPath === null) {
        fwrite(STDERR, "Warning: sink_registry.json not found, using built-in fallback definitions\n");
        return loadFallbackSinks();
    }

    $raw = file_get_contents($registryPath);
    $registry = json_decode($raw, true);
    if ($registry === null) {
        fwrite(STDERR, "Warning: sink_registry.json parse error, using built-in fallback definitions\n");
        return loadFallbackSinks();
    }
    ...
    foreach ($registry['categories'] as $categoryName => $category) {
        if (!isset($category['sub_categories'])) {
            continue;
        }
        foreach ($category['sub_categories'] as $sub) {
            if (isset($sub['functions'])) {
                foreach ($sub['functions'] as $func) {
                    $sinkDefs[$func['name']] = $categoryName;
                }
            }
            if (isset($sub['methods'])) {
                foreach ($sub['methods'] as $method) {
                    $methodSinks[$method['name']] = $categoryName;
                }
            }
            if (isset($sub['static_methods'])) {
                foreach ($sub['static_methods'] as $static) {
                    $staticSinks[$static['name']] = $categoryName;
                }
            }
        }
    }

    return [$sinkDefs, $methodSinks, $staticSinks];
}

function loadFallbackSinks(): array
{
    $SINK_DEFINITIONS = [
        'eval' => 'RCE', 'assert' => 'RCE', 'preg_replace' => 'RCE',
        'create_function' => 'RCE', 'system' => 'RCE', 'exec' => 'RCE',
        'passthru' => 'RCE', 'shell_exec' => 'RCE', 'popen' => 'RCE',
        'proc_open' => 'RCE', 'pcntl_exec' => 'RCE',
        'call_user_func' => 'RCE', 'call_user_func_array' => 'RCE',
        'array_map' => 'RCE', 'array_filter' => 'RCE',
        'usort' => 'RCE', 'uasort' => 'RCE', 'uksort' => 'RCE',
        'array_walk' => 'RCE', 'extract' => 'RCE', 'parse_str' => 'RCE',
        'mb_parse_str' => 'RCE',
        'mysql_query' => 'SQLi', 'pg_query' => 'SQLi', 'sqlite_query' => 'SQLi',
        'highlight_file' => 'LFI', 'show_source' => 'LFI',
        'file_get_contents' => 'LFI', 'readfile' => 'LFI', 'fread' => 'LFI',
        'file' => 'LFI', 'fpassthru' => 'LFI', 'fopen' => 'LFI',
        'file_put_contents' => 'FileWrite', 'fwrite' => 'FileWrite',
        'fputs' => 'FileWrite', 'move_uploaded_file' => 'FileWrite',
        'copy' => 'FileWrite', 'rename' => 'FileWrite',
        'unserialize' => 'Deserialization',
        'curl_exec' => 'SSRF', 'curl_multi_exec' => 'SSRF',
        'get_headers' => 'SSRF', 'getimagesize' => 'SSRF',
        'printf' => 'XSS', 'vprintf' => 'XSS',
        'simplexml_load_string' => 'XXE', 'simplexml_load_file' => 'XXE',
        'libxml_disable_entity_loader' => 'XXE',
        'update_option' => 'WordPress', 'update_user_meta' => 'WordPress',
        'wp_set_auth_cookie' => 'WordPress', 'do_shortcode' => 'WordPress',
        'wp_remote_get' => 'WordPress', 'wp_remote_post' => 'WordPress',
        'is_admin' => 'WordPress',
        'md5' => 'Crypto', 'sha1' => 'Crypto', 'rand' => 'Crypto',
        'mt_rand' => 'Crypto', 'mcrypt_encrypt' => 'Crypto',
        'mcrypt_decrypt' => 'Crypto', 'mcrypt_cbc' => 'Crypto',
        'mcrypt_cfb' => 'Crypto', 'mcrypt_ecb' => 'Crypto',
        'mcrypt_ofb' => 'Crypto', 'mcrypt_create_iv' => 'Crypto',
        'mcrypt_generic' => 'Crypto', 'mcrypt_module_open' => 'Crypto',
        'openssl_encrypt' => 'Crypto',
    ];

    $METHOD_SINKS = [
        'query' => 'SQLi', 'exec' => 'SQLi', 'multi_query' => 'SQLi',
        'raw' => 'SQLi', 'whereRaw' => 'SQLi', 'havingRaw' => 'SQLi',
        'orderByRaw' => 'SQLi', 'selectRaw' => 'SQLi', 'groupByRaw' => 'SQLi',
        'findBySql' => 'SQLi', 'extractTo' => 'FileWrite',
        'loadXML' => 'XXE', 'load' => 'XXE',
        'find' => 'NoSQL', 'findOne' => 'NoSQL', 'aggregate' => 'NoSQL',
        'updateOne' => 'NoSQL', 'deleteMany' => 'NoSQL', 'insertOne' => 'NoSQL',
        'eval' => 'NoSQL', 'rawCommand' => 'NoSQL',
        'get_results' => 'WordPress', 'get_row' => 'WordPress',
        'get_var' => 'WordPress', 'executeQuery' => 'GraphQL',
    ];

    $STATIC_SINKS = [
        'DB::raw' => 'SQLi', 'DB::select' => 'SQLi', 'DB::statement' => 'SQLi',
        'Db::query' => 'SQLi', 'Db::execute' => 'SQLi',
        'Model::create' => 'MassAssignment',
        'GraphQL::executeQuery' => 'GraphQL',
        'GraphQLServer::executeQuery' => 'GraphQL',
    ];

    return [$SINK_DEFINITIONS, $METHOD_SINKS, $STATIC_SINKS];
}

[$SINK_DEFINITIONS, $METHOD_SINKS, $STATIC_SINKS] = loadSinkRegistry();
