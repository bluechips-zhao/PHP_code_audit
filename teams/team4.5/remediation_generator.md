...
### Procedure E: Patch Auto-Verification (NEW)
For each generated patch, verify that it actually fixes the vulnerability:

| Step | Action | Command/Method |
|------|--------|----------------|
| E-1 | Apply patch to source code | `cd $TARGET_PATH && git apply $WORK_DIR/修复补丁/{sink_id}.patch` |
| E-2 | Verify patch applied cleanly | Check `git apply` exit code = 0 |
| E-3 | Re-send attack payload | Replay the successful payload from `exploits/{sink_id}.json` using the PoC script or curl |
| E-4 | Check vulnerability is fixed | Expected: HTTP response does NOT contain evidence of exploitation (e.g., no command output, no DB error, no file content) |
| E-5 | Check no regression | Verify the endpoint still returns valid responses for legitimate requests |
| E-6 | Roll back patch | `cd $TARGET_PATH && git apply -R $WORK_DIR/修复补丁/{sink_id}.patch` |
| E-7 | Verify rollback clean | Check `git apply -R` exit code = 0, verify `git diff` shows no changes |

**Verification result classification**:
| Result | Condition | verification_status Value |
|--------|-----------|--------------------------|
| Verified | Patch applied + vulnerability blocked + no regression + rollback clean | `verified` |
| Partially Verified | Patch applied + vulnerability blocked but regression detected | `partial_regression` |
| Not Fixed | Patch applied but vulnerability still exploitable | `not_fixed` |
| Apply Failed | `git apply --check` fails or patch conflicts | `apply_failed` |
| Rollback Failed | Rollback did not restore original state | `rollback_failed` |

**🚨 CRITICAL**: If rollback fails (E-7), MUST immediately:
1. `cd $TARGET_PATH && git checkout -- .` to restore all files
2. Log warning in remediation_summary.json
3. Set `rollback_clean: false` for this patch

**Skip conditions** (do not run verification when):
- Target project is not a git repository (`git rev-parse --is-inside-work-tree` fails)
- Docker container is not running (cannot re-send payloads)
- `exploits/{sink_id}.json` has no successful payload (`status != "confirmed"`)
- Patch is for a configuration change (cannot auto-verify config patches)

### Procedure F: Verification Recommendations
| Field | Fill-in Value |
|-------|--------------|
...
```json
{
  ...
  "patches": [{
    ...
    "verification_status": "string (verified|partial_regression|not_fixed|apply_failed|rollback_failed|skipped)",
    "verification_detail": "string (description of verification result or skip reason)",
    "rollback_clean": "boolean (whether source code was restored to original state)",
