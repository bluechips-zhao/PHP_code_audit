...
## 25. PHP 8.x New Feature Sink

> PHP 8.0+ introduces new language features that create novel attack surfaces. These sinks are version-dependent — only applicable when the target runs PHP 8.0+.

### Named Arguments Injection (PHP 8.0+)
| Pattern | Risk Level | Context Pattern Description |
|------|----------|----------------|
| `htmlspecialchars($input, encoding: $userControlled)` | High | Named arguments allow overriding security-relevant default parameters (e.g., changing encoding to '7bit' bypasses XSS protection). |
| `setcookie($name, $value, path: $userInput)` | High | User-controlled named arguments can set cookie path/domain to sensitive values. |
| Any function call with user-controllable named argument overriding a security default | High | General pattern: if user can specify `parameter_name: value`, they can skip intermediate parameters and override defaults. |

### First-Class Callable Syntax (PHP 8.1+)
| Pattern | Risk Level | Context Pattern Description |
|------|----------|----------------|
| `$func(...)` where `$func` is user-controllable | Critical | Creates Closure from any callable string (e.g., `'system'(...)`), enabling RCE. |
| `$userInput(...)` used as callback | Critical | First-class callable bypasses `is_string()` checks on callbacks — returns Closure object, not string. |
| Dynamic callable creation from user input | High | `$callable = $_GET['action'](...); $callable($arg);` — direct RCE vector. |

### Fiber Concurrency (PHP 8.1+)
| Pattern | Risk Level | Context Pattern Description |
|------|----------|----------------|
| `Fiber::suspend()` between security check and protected operation | High | TOCTOU race condition — state may change between check and use across Fiber boundaries. |
| Shared mutable state accessed across Fiber yield points | Medium | Data consistency issues when multiple Fibers access shared state. |
| `Fiber::start()` / `Fiber::resume()` with shared resource access | Medium | Cooperative concurrency can create windows for state manipulation. |

### Enum Type Confusion (PHP 8.1+)
| Pattern | Risk Level | Context Pattern Description |
|------|----------|----------------|
| `EnumName::from($userInput)` without pre-validation | Medium | ValueError exception message may disclose valid enum values (information disclosure). |
| `EnumName::tryFrom($userInput)` without null check | High | Returns null for invalid values — if not checked, may bypass authorization (null in loose comparison). |
| Enum used in `match()` without default branch | Medium | `UnhandledMatchError` when null from tryFrom() reaches match expression. |

### Attribute Injection (PHP 8.0+)
| Pattern | Risk Level | Context Pattern Description |
|------|----------|----------------|
| `#[RequireRole(role: $dynamic)]` with user-influenced value | High | If framework evaluates attributes at runtime, attacker can bypass authorization. |
| `#[Route(path: $userDefined)]` | High | User-controlled routing attribute may create unauthorized endpoints. |
| `ReflectionAttribute::newInstance()` with unvalidated args | Medium | Reflection-based attribute instantiation with user-controlled constructor arguments. |

### Match Expression (PHP 8.0+)
| Pattern | Risk Level | Context Pattern Description |
|------|----------|----------------|
| `match((int)$userInput)` with type coercion before match | High | Coercion before match undermines strict comparison: `"1admin"` → `1` matches admin branch. |
| `match($userInput)` without default branch | Medium | `UnhandledMatchError` for unexpected values — may crash or expose internal logic. |

> **Canonical source**: The complete sink function registry with version-specific metadata is maintained in `schemas/sink_registry.json`. This document is the human-readable reference; `sink_finder.php` loads from `sink_registry.json` at runtime.
