...
## PHP 8.x New Feature Security Patterns

### Named Arguments Injection (PHP 8.0+)

#### How It Works

PHP 8.0 introduced named arguments, allowing function parameters to be passed by name rather than position. This enables attackers to skip intermediate parameters or pass values to parameters that were intended to be hardcoded.

#### Detection Method

```
Search for functions where:
1. User input is passed as positional argument
2. The function has security-relevant parameters after the user-controlled one
3. Default values of skipped parameters may weaken security
```

#### Payload Examples

```php
// Vulnerable code: developer expects only $data to be user-controlled
htmlspecialchars($user_input);

// Attack via named arguments: override the $encoding parameter
htmlspecialchars($user_input, encoding: '7bit');
// 7bit encoding does NOT encode <>"'& → XSS bypass

// Another example: PDO query with named argument override
$stmt = $pdo->query($sql, mode: PDO::FETCH_LAZY);
// FETCH_LAZY may change how data is accessed, potentially bypassing type checks

// Real-world: setcookie() parameter injection
setcookie("session", $user_token, path: "/admin");
// If $user_token is controllable, attacker can set path to /admin
// gaining access to cookies in the admin directory
```

#### Key Insight Summary

> Named arguments allow attackers to override default parameter values that developers rely on for security. When auditing PHP 8.0+ code, check if user input could be passed as named arguments to functions with security-sensitive defaults.
...
PHP 8.1 introduced first-class callable syntax (`$func(...)`) which creates a Closure from any callable. This simplifies dynamic function invocation and can bypass callback restrictions.

#### Detection Method

```
Search for:
- $func(...) syntax used with user-controllable function names
- Callable creation from user input: $cb = $input(...)
- Dynamic method invocation via first-class callables
```

#### Payload Examples

```php
// Vulnerable code: user controls which function is called
$action = $_GET['action']; // e.g., "system"
$callable = $action(...);   // Creates Closure from "system"
$callable('id');            // Executes system('id')

// Bypass callback restrictions that check for string callables
// Old code: if (is_string($callback)) { /* block */ }
// New bypass: $callback is now a Closure object, not a string
$dangerous = 'system'(...); // Returns Closure, not string
// is_string($dangerous) → false → bypasses string-based checks

// Combined with variable functions
$func = $_GET['f']; // "exec"
$result = $func(...); // Creates and calls exec(...)
```

#### Key Insight Summary

> First-class callables convert string function names into Closure objects, potentially bypassing security checks that only inspect string-type callables. Audit for any user-controlled callable creation.

---

### Fiber Concurrency Issues (PHP 8.1+)

#### How It Works

PHP 8.1 introduced Fibers for cooperative multitasking. While PHP Fibers are not truly parallel (they are cooperative, not preemptive), they can cause TOCTOU (Time-of-Check-to-Time-of-Use) issues when a Fiber yields between a security check and the protected operation.

#### Detection Method

```
Search for:
- Fiber::start() / Fiber::resume() / Fiber::suspend()
- Security checks followed by Fiber::suspend() before the protected operation
- Shared state modifications across Fiber boundaries
```

#### Payload Examples

```php
// Vulnerable code: TOCTOU via Fiber
function processPayment($amount) {
    if ($amount > 1000) {
        Fiber::suspend(); // Yield control — balance may change
        // After resume, $amount is still the old value
        // but account balance may have been modified by another Fiber
    }
...
$fiber1 = new Fiber(function() {
    $balance = getBalance(); // Check: balance = 1000
    Fiber::suspend();        // Attacker triggers another deduction here
    withdraw(1000);          // Use: still succeeds, but balance is now 0
});

$fiber2 = new Fiber(function() {
    withdraw(1000);          // Deducts during fiber1's suspend
});
```

#### Key Insight Summary

> Fibers introduce cooperative concurrency to PHP. Any security check followed by a Fiber::suspend() before the protected operation is a potential TOCTOU vulnerability. Audit all Fiber yield points for state consistency.

---

### Enum::from() Type Confusion (PHP 8.1+)

#### How It Works

PHP 8.1 Enums have `from()` (throws ValueError on invalid value) and `tryFrom()` (returns null on invalid value). If `tryFrom()` result is not null-checked, or if `from()` is used with user input without pre-validation, it can lead to unexpected behavior.

#### Detection Method

```
Search for:
- EnumName::from($user_input) without prior validation
- EnumName::tryFrom($user_input) without null check
- Enums used in authorization/role checks
```

#### Payload Examples

```php
// Vulnerable code: role enum used in authorization
enum Role: string {
    case Admin = 'admin';
    case User = 'user';
    case Guest = 'guest';
...
// Attack 1: tryFrom without null check
$role = Role::tryFrom($_GET['role']); // Returns null for invalid values
if ($role) { // null is falsy, but what if developer uses == ?
    grantAccess($role);
}

// Attack 2: from() throws ValueError — may reveal enum values in error message
try {
    $role = Role::from($_GET['role']); // ValueError: "admin" is not a valid backing value
} catch (ValueError $e) {
    echo $e->getMessage(); // Information disclosure: reveals valid enum values
}

// Attack 3: Enum in match expression without default
match ($role) {
    Role::Admin => adminPanel(),
    Role::User => userPanel(),
    // Missing default → UnhandledMatchError if null from tryFrom()
};
```

#### Key Insight Summary

> Enum `from()`/`tryFrom()` with user input can cause type confusion or information disclosure. Always validate input before `from()`, always null-check `tryFrom()` results, and never expose ValueError messages.

---

### Attribute Injection (PHP 8.0+)

#### How It Works

PHP 8.0 Attributes (annotations) are metadata declared with `#[...]` syntax. Frameworks like Laravel, Symfony, and Drupal use attributes for routing, middleware, validation, and authorization. If user-controlled data flows into attribute arguments, it may affect application behavior.

#### Detection Method

```
Search for:
- Custom Attribute classes that accept user-influenced parameters
- ReflectionAttribute::newInstance() with unvalidated arguments
- Framework routing attributes with dynamic parameters
- Attribute-based access control that reads from user input
```

#### Payload Examples

```php
// Vulnerable: attribute-based authorization with dynamic value
#[RequireRole(role: $_GET['required_role'])]
public function adminAction() {
    // If the framework evaluates the attribute at runtime,
    // attacker can set required_role to 'guest' to bypass
}

// Vulnerable: custom attribute with user-controlled constructor args
#[Route(path: $userDefinedPath)]
public function dynamicRoute() {
    // Attacker controls the route path → potential unauthorized access
}

// Reflection-based attribute abuse
$reflection = new ReflectionMethod($controller, $action);
$attrs = $reflection->getAttributes();
foreach ($attrs as $attr) {
    $instance = $attr->newInstance(); // If attribute constructor has side effects
}
```

#### Key Insight Summary

> Attributes are typically static metadata, but when their arguments are dynamically constructed from user input, they can bypass authorization, routing, or validation. Audit for any attribute whose constructor arguments include user-controllable data.

---

### Match Expression Coercion (PHP 8.0+)

#### How It Works

PHP 8.0's `match` expression uses strict comparison (`===`), which is safer than `switch`. However, if the match subject is loosely typed (e.g., from `$_GET` which is always a string), the strict comparison may fail unexpectedly, causing the default branch to execute — which may not be properly handled.

#### Detection Method

```
Search for:
- match expressions with user input as the subject
- Missing default branch in match expressions
- Type coercion before match (e.g., (int)$_GET['action'])
```

#### Payload Examples

```php
// Vulnerable code: missing default branch
$result = match ($_GET['action']) {
    'delete' => deleteUser(),
    'update' => updateUser(),
    // No default → UnhandledMatchError for any other value
    // Error may expose internal logic or crash the application
};

// Vulnerable: type coercion before match bypasses strict comparison
$action = (int)$_GET['action']; // "1admin" → 1
$result = match ($action) {
    0 => guestAccess(),
    1 => adminAccess(),  // "1admin" coerced to 1 → admin access
    2 => userAccess(),
};

// Safe pattern: explicit type validation + default
...
> `match` is strict by default, but type coercion before the match can undermine this safety. Always validate the type of the match subject and provide a default branch.

---

### PHP 8.x Audit Checklist Addendum

```
□ Can named arguments override security-relevant default parameters?
□ Is first-class callable syntax used with user-controllable function names?
□ Do Fiber yield points create TOCTOU windows between checks and operations?
□ Are Enum::from()/tryFrom() results properly validated and null-checked?
□ Do any Attribute constructors accept user-controllable arguments?
□ Does match expression subject undergo type coercion before matching?
□ Is the PHP 8.0+ strict comparison behavior change accounted for in == checks?
□ Are Fiber-based concurrency patterns tested for state consistency?
□ Does ValueError from Enum::from() leak valid enum values?
□ Are Attribute-based access controls evaluated at compile-time or runtime?
```
